...
local Sound = {
        Button = "rbxassetid://9126031699",
        Section = "rbxassetid://5120879183",
        Slider = "rbxassetid://9126031699"
        }

local MainWidth = 370
local MainHeight = 350
local SectionSize = 175
...
local function centerGui()
    ...
    local offsetY = (screenHeight - main.Size.Y.Offset) / 3
    
    -- Menyesuaikan posisi GUI
    main.Position = UDim2.new(0, offsetX, 0, offsetY)
end

-- Panggil fungsi centerGui untuk pertama kali menengahkan GUI
centerGui()
...
topbar.Size = UDim2.new(0, MainWidth, 0, 34)
topbar.Parent = main
...
topbarLine.Size = UDim2.new(0, MainWidth, 0, 1)
topbarLine.ZIndex = 2
...
closeButton.Position = UDim2.new(0.910, 3, 0.235, 0)
closeButton.Size = UDim2.new(0, 17, 0, 17)
closeButton.Parent = topbar

local closeSound = Instance.new("Sound")
    closeSound.SoundId = Sound.Button  -- Ganti dengan asset ID suara yang Anda inginkan
...
local minimSound = Instance.new("Sound")
    minimSound.SoundId = Sound.Button  -- Ganti dengan asset ID suara yang Anda inginkan
    minimSound.Parent = minimizeButton
    minimSound.Volume = 0.4
...
minimizeButton.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(main, TweenInfo.new(.125, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), {Size = Minimized and UDim2.new(0, MainWidth, 0, 34) or UDim2.new(0, MainWidth, 0, MainHeight)}):Play()
    if not Minimized then
        wait(.125)
        topbarLine.Visible = true
    end
    for _,v in next, main:GetChildren() do
        if v.Name ~= "Topbar" and (v:IsA("Frame") or v:IsA("ScrollingFrame")) then
...
containers.Position = UDim2.new(0, 1, 0.097, 0)
...
section.Size = UDim2.new(0, SectionSize, 0, 33)
...
sectionFrame.Size = UDim2.new(0, SectionSize, 0, 28)
...
sectionButton.Size = UDim2.new(0, SectionSize, 0, 28)
sectionButton.Parent = sectionFrame
sectionButton.ZIndex = Sections + 1

local sectionSound = Instance.new("Sound")
    sectionSound.SoundId = Sound.Section  -- Ganti dengan asset ID suara yang Anda inginkan
...
containerHolder.Size = UDim2.new(0, SectionSize, 0, 0)
...
itemContainer.Size = UDim2.new(0, SectionSize, 0, 0)
...
function sectiontable:Select()
    ...
    section.Size = OpenedSec and UDim2.new(0, SectionSize, 0, SectionY + 5) or UDim2.new(0, SectionSize, 0, 33)
    sectionFrame.Size = OpenedSec and UDim2.new(0, SectionSize, 0, SectionY) or UDim2.new(0, SectionSize, 0, 28)
    containerHolder.Position = OpenedSec and UDim2.new(0, 0, 0, 28) or UDim2.new(0, 0, 0, 0)
    containerHolder.Size = OpenedSec and UDim2.new(0, SectionSize, 0, ContainerY) or UDim2.new(0, SectionSize, 0, 0)
    sectionIcon.Rotation = OpenedSec and -90 or 90
    sectionIcon.Position = OpenedSec and UDim2.new(0, 156, 0, 5) or UDim2.new(0, 155, 0, 5)
    itemContainer.Size = OpenedSec and UDim2.new(0, SectionSize, 0, ContainerY) or UDim2.new(0, SectionSize, 0, 0)
end
...
button.Parent = itemContainer
    ...
    buttonSound.SoundId = Sound.Button  -- Ganti dengan asset ID suara yang Anda inginkan
    buttonSound.Parent = button
...
local toggleSound = Instance.new("Sound")
    toggleSound.SoundId = Sound.Slider  -- Ganti dengan asset ID suara yang Anda inginkan
...
local dragSound = Instance.new("Sound")
    dragSound.SoundId = Sound.Slider  -- Ganti dengan asset ID suara yang Anda inginkan
...
local dropSound = Instance.new("Sound")
    dropSound.SoundId = Sound.Slider  -- Ganti dengan asset ID suara yang Anda inginkan
...
dropdownElementText.TextColor3 = Theme.ElementText
...
function insidedropdown:Clear()
    for _, v in pairs(dropdownContainer:GetChildren()) do
        if v:IsA("Frame") and v:FindFirstChild("dropdownElementText") then
